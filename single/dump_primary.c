...
int main(int argc, char *argv[])
{
    ...
    u_int32_t numrecs = 0;
    //DB_BTREE_STAT *stat;
    char *sql_query = "update invpat_2_sort set Invnum_N = ? where invnum = ?;";
    ...
    //free(stat);
    sqlite3_open("sqlite_dbs/invpatC.sqlite3", &sql_db);
    ...
    while(DB_NOTFOUND != cur->pget(cur, &key, &pkey, &data,
        ...
        do{
            ...
            if(strncmp(((DbRecord*)data.data)->Invnum_N, "", 1)){
                ...
                if((sql_ret = sqlite3_bind_text(ppStmt, 2, ((DbRecord *)data.data)->Invnum, -1, SQLITE_STATIC))!=0)
                    printf("Bind complaint: %d\n", sql_ret);
