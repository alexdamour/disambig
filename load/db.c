 ...
 */
int
sqlite_db_env_open(const char *home)
{
	int ret;
	 ...
	 */
	if ((ret = dbenv->set_cachesize(dbenv,CACHE_GIGS, CACHE_BYTES, 1)) != 0) {
    ...
    */
	...
	//if ((ret = sqlite_db_secondary_open()) != 0)
	//	return (1);

	return (0);
}

int sqlite_db_primary_open(DB **dbp, const char* fname, DBTYPE type, u_int32_t pagesize,
                            u_int32_t open_flag, u_int32_t dup_flag,
                            int(*compare)(DB*, const DBT*, const DBT*)){
    int ret = 0;
    ret = db_create(dbp, dbenv, 0);
    ret = (*dbp)->set_pagesize(*dbp, pagesize);
    if(type == DB_QUEUE){
        ret = (*dbp)->set_re_len(*dbp, BLOCK_TAG_LEN);
    }
    if(compare != NULL)
        ret = (*dbp)->set_bt_compare(*dbp, compare);
    if(dup_flag)
        ret = (*dbp)->set_flags(*dbp, dup_flag);
    ret = (*dbp)->open(*dbp, NULL, fname, NULL, type, open_flag, 0);
    return(ret);
}

int sqlite_db_secondary_open(DB *prim, DB **sdb, char *fname, u_int32_t pagesize,
                             u_int32_t dup_flag, int(*callback)(DB*, const DBT*, const DBT*, DBT*),
                             int(*compare)(DB*, const DBT*, const DBT*)){
    int ret=0;
    ret = db_create(sdb, dbenv, 0);
    ret = (*sdb)->set_pagesize(*sdb, pagesize);
    if(dup_flag)
        ret = (*sdb)->set_flags(*sdb, dup_flag);
    if(compare != NULL)
        ret = (*sdb)->set_dup_compare(*sdb, compare);
    ret = (*sdb)->open(*sdb, NULL, fname, NULL, DB_BTREE, DB_CREATE, 0);
    ret = (*sdb)->associate(prim, NULL, *sdb, callback, DB_CREATE);
...
//			return (1);
//		}
//        //This will be used to figure out which key we want to use
//        // to associate the secondary database.
//		sdb->app_private = f;
//
